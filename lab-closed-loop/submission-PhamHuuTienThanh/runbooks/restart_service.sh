#!/usr/bin/env bash
# restart_service.sh — restart (or start) a Ronki service container.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "[restart_service] Unknown arg: $1"; exit 1 ;;
  esac
done

if [[ -z "$SERVICE" ]]; then
  echo "[restart_service] ERROR: --service <name> required"; exit 1
fi
